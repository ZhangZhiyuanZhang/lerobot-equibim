...
def mirror_state(state: torch.Tensor):
    """
    state: (B, T, 12)
    [L_arm(5), L_grip(1), R_arm(5), R_grip(1)]
    """
    L = state[..., :6]
    R = state[..., 6:]

    sign = JOINT_SIGN.to(state.device)

    Lm = R * sign
    Rm = L * sign

    return torch.cat([Lm, Rm], dim=-1)


def mirror_action(action: torch.Tensor):
    """
    action: (B, T, 12)
    """
    L = action[..., :6]
    ...
    sign = JOINT_SIGN.to(action.device)
...
def mirror_images(images):
    """
    images:
      - ACT: list[Tensor], each (B, T, C, H, W)
      - Diffusion: Tensor (B, T, num_cams, C, H, W)
    """
    if isinstance(images, list):
        # ACT style: list of camera tensors
        return [torch.flip(img, dims=[-1]) for img in images]
    else:
...
class ACTPolicy(PreTrainedPolicy):
    ...
    def forward(self, batch: dict[str, Tensor]) -> tuple[Tensor, dict]:
        ...
        l1_loss = (
            F.l1_loss(batch[ACTION], actions_hat, reduction="none") * valid
        ).mean()
        ...
        # Symmetry loss
        # ======================
        if self.training and self.config.use_sym_loss:

            batch_sym = {}
            for k, v in batch.items():
                if k == OBS_STATE:
                    batch_sym[k] = mirror_state(v)
                elif k == OBS_IMAGES:
                    batch_sym[k] = mirror_images(v)
                else:
                    batch_sym[k] = v
